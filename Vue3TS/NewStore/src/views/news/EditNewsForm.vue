    ...
    <div class="editor-container">
      ...
      <div class="input-row">
        <div class="input-group">
          ...
          <div class="form-item">
            <button class="button-submit" @click="updateNews">保存新闻</button>
          </div>
        </div>
  
        <div v-if="headImgUrl" class="button-group">
            <apiUploadHeadImage 
            @send-param="reviewParam"
...
<script lang="ts">
  ...
  import { onBeforeUnmount, ref, shallowRef, onMounted, computed, toRefs, nextTick, watch } from 'vue'
  ...
  import { ArticleStatus, type UpdateData, apiUpdateNews } from '@/api/news';
  ...
  export default {
    ...
    setup(props) {
      ...
      const editorConfig: Partial<IEditorConfig> = {
          ...
          MENU_CONF: {
            uploadImage: {
                  maxFileSize: 15 * 1024 * 1024, // 设置15MB限制
      ...
      watch(
        ...
        { deep: true, immediate: true } // 立即监听并处理初始值
        );

      const handleHeadImageUpdate = (newUrl: string) => {
        headImgUrl.value = newUrl
        // 强制刷新组件
